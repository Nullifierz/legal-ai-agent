...
export class ChatComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    this.artifactService
        ...
        .subscribe({
          next: (res) => {
            ...
            // Intercept text/markdown artifacts → open in Canvas panel
            if (mimeType === 'text/markdown' || mimeType === 'text/plain') {
              const binaryString = atob(fixBase64String(data));
              ...
              if (autoOpenCanvas) {
                this.openCanvas(decodedContent, artifactId);
              }

              // Attach canvas artifact info to the message for inline card rendering
              (message as any).canvasArtifact = {
                title: artifactId,
                content: decodedContent,
                timestamp: new Date(),
              };
              // Remove the placeholder inlineData so the template doesn't render an empty image
              delete (message as any).inlineData;
              // Force change detection by updating the messages signal
  ...
  openCanvas(content: string, title: string) {
  ...
  onCanvasContentChange(updatedContent: string) {
    this.canvasContent = updatedContent;
  }
  ...
  private populateMessages(
    ...
    events.forEach((event: any) => {
      ...
      if (event.author === 'user') {
        ...
        // Process artifact deltas for session reload
        const userArtifactDelta = event.actions?.artifactDelta || event.actions?.artifact_delta;
        if (userArtifactDelta) {
          for (const key in userArtifactDelta) {
            if (userArtifactDelta.hasOwnProperty(key)) {
              this.renderArtifact(key, userArtifactDelta[key], reverseOrder, false);
            }
          }
        }
      } else {
        ...
        // Process artifact deltas for session reload
        const botArtifactDelta = event.actions?.artifactDelta || event.actions?.artifact_delta;
        if (botArtifactDelta) {
          for (const key in botArtifactDelta) {
            if (botArtifactDelta.hasOwnProperty(key)) {
              this.renderArtifact(key, botArtifactDelta[key], reverseOrder, false);
            }
          }
        }
      }
  ...
  protected updateWithSelectedSession(session: Session) {
    ...
    session.events.forEach((event: any) => {
      if (event.author === 'user') {
        ...
        // Process artifact deltas for session reload
        const userArtifactDelta2 = event.actions?.artifactDelta || event.actions?.artifact_delta;
        if (userArtifactDelta2) {
          for (const key in userArtifactDelta2) {
            if (userArtifactDelta2.hasOwnProperty(key)) {
              this.renderArtifact(key, userArtifactDelta2[key], false, false);
            }
          }
        }
        ...
        // Process artifact deltas for session reload
        const botArtifactDelta2 = event.actions?.artifactDelta || event.actions?.artifact_delta;
        if (botArtifactDelta2) {
          for (const key in botArtifactDelta2) {
            if (botArtifactDelta2.hasOwnProperty(key)) {
              this.renderArtifact(key, botArtifactDelta2[key], false, false);
            }
          }
        }
